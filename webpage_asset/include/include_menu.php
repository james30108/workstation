                ...
                <div class="row">
                    <div class="col-12 d-flex">
                        <nav class="navbar navbar-expand-lg d-none d-sm-block ms-sm-auto">
                            <ul class="navbar-nav">
                                <li class="dropdown"><a class="nav-link text-dark" href="?page=shop">สินค้า</a></li>
                                <li class="dropdown"><a class="nav-link text-dark" href="?page=package">แพ็กเกจพิเศษ</a></li>
                                <li class="dropdown"><a class="nav-link text-dark" href="?page=thread">ประชาสัมพันธ์</a></li>
                                <!--<li class="dropdown mb-3"><a class="nav-link text-dark" href="?page=track">ตรวจสอบคำสั่งซื้อ</a></li>-->
                                <li class="dropdown"><a class="nav-link text-dark" href="?page=contact">ติดต่อเรา</a></li>
                            </ul>
                        </nav>
                        ...
                        <?php if (isset($data_check_login)) { 
                            ...
                            $query_cart = mysqli_query($connect, "SELECT * FROM system_cart WHERE (cart_member_id = '$buyer_id')");
                            $cart       = mysqli_num_rows($query_cart);
                            $cart_canvas= ($cart != 0) ? "#cart_canvas" : "";
                            ?>
                            <a data-bs-toggle="offcanvas" href="<?php echo $cart_canvas ?>" role="button" aria-controls="offcanvasExample" class="d-flex align-items-center me-sm-3 ms-auto ms-sm-0">
                                ...
                                <?php echo $cart != 0 ? "<span class='badge rounded-pill bg-danger'>$cart</span>" : false; ?>
                            ...
                            <a data-bs-toggle="offcanvas" href="#profile_canvas" role="button" aria-controls="offcanvasExample" class="d-flex align-items-center">
                                ...
                                <?php
                                $query_contact = mysqli_query($connect, "SELECT * FROM system_contact WHERE contact_type NOT IN (0, 2) AND (contact_buyer = '$buyer_id') AND (contact_status = 0)");
                                $contact       = mysqli_num_rows($query_contact);
                                echo $contact != 0 ? "<span class='badge rounded-pill bg-danger'>$contact</span>" : false; ?>
                            </a>
                        <?php } else { ?>
                            <a href="visitor_login.php" class="d-flex align-items-center">
                                เข้าสู่ระบบ <i class="bx bx-log-in" style="font-size: 20px;"></i>
                            </a>   
                        <?php } ?>              
                    </div>
                </div>
...
<div class="offcanvas offcanvas-end" tabindex="-1" id="cart_canvas" aria-labelledby="offcanvasExampleLabel">
    <div class="offcanvas-header">
        <h5 class="offcanvas-title" id="offcanvasExampleLabel">ตะกร้าสินค้า</h5>
        ...
        if ($count_cart > 0) { ?>
            <div class="">
                    ...
                    <div class="position-relative">
                        <div class="row">
                            <div class="col-4 ms-auto">
                                <a class="pull-left" href="?page=product_single&product_id=<?php echo $data['product_id'] ?>">
                                    <img src="<?php echo $image ?>" alt="Image Cover" class="img-thumbnail border border-1">
                                </a>
                            </div>
                            <div class="col-8 text-start">
                                <h6 class="mb-0"><a href="?page=product_single&product_id=<?php echo $data['product_id'] ?>"><?php echo $data['product_name'] ?></a></h6>
                                <div class="text-secondary mb-1">
                                    <span><?php echo $quantity ?> x</span>
                                    <span><?php echo number_format($data['product_price']) ?></span>
                                </div>
                                <h6><?php echo number_format($price) ?> บาท</h6>
                                <a href="dashboard/process/setting_visitor_buy.php?action=delete_cart&cart_id=<?php echo $data['cart_id'] ?>" class="text-decoration-underline small text-danger">ลบ</a>
                            </div>
                        </div>
                    </div>
                    <hr>
                ...
                <div class="cart-summary">
                    <span>ราคารวม</span>
                    <span class="total-price"><?php echo number_format($total) ?> บาท</span>
                </div>
                <ul class="text-center cart-buttons">
                    <li><a href="?page=cart" class="btn btn-small">ดูตะกร้าสินค้า</a></li>
                    <li><a href="?page=checkout" class="btn btn-small btn-solid-border">ชำระเงิน</a></li>
                </ul>
            </div>
        <?php } ?>
    </div>
...
<div class="offcanvas offcanvas-end" tabindex="-1" id="profile_canvas" aria-labelledby="offcanvasExampleLabel">
    ...
    <div class="offcanvas-body">
        ...
        <ul class="list-group list-group-flush text-start mt-5">
            <li class="mb-3"><a href="?page=profile">ข้อมูลส่วนตัว</a></li>
            <li class="mb-3"><a href="?page=profile&action=order">การสั่งซื้อสินค้า</a></li>
            <li class="mb-3"><a href="?page=message">การแจ้งเตือน
            <?php if ($contact != 0) { ?>
...
<div class="offcanvas offcanvas-start" tabindex="-1" id="menu_canvas" aria-labelledby="offcanvasExampleLabel">
    ...
    <div class="offcanvas-body">
        <ul class="list-group list-group-flush text-start mt-5">
            <li class="mb-3"><a href="index.php">หน้าหลัก</a></li>
            <li class="mb-3"><a href="?page=shop">สินค้า</a></li>
            <li class="mb-3"><a href="?page=package">แพ็กเกจพิเศษ</a></li>
            <li class="mb-3"><a href="?page=thread">ประชาสัมพันธ์</a></li>
            <li class="mb-3"><a href="?page=contact">ติดต่อเรา</a></li>
        </ul>
    </div>
</div>