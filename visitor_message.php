...
<div class="container">
    <?php if (!isset($_GET['action'])) { 
        $query = mysqli_query($connect, "SELECT * FROM system_contact WHERE (contact_buyer = '$buyer_id') AND ((contact_type = 1) OR (contact_type = 3)) ORDER BY contact_id DESC");
    ...
    <?php } } else { echo "<p class='m-5'>$l_notfound</p>"; }
        ...
        <div class="row p-5">
            ...
            <?php if ($data['contact_type'] == 3) { 
                ...
                $query = mysqli_query($connect, "SELECT * FROM system_order WHERE order_id = '$contact_detail' ");
                $data  = mysqli_fetch_array($query); 
                ...
                <table class="table table-bordered">
                    <tbody>
                        <tr>
                            ...
                            <td><a href="?page=profile&action=order_detail&order_id=<?php echo $order_id ?>" class="text-decoration-underline text-primary" target="_blank"><?php echo $order_code ?></a></td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td><?php echo $order_track_name ?></td>
                        </tr>
                        <tr>
                            <th><?php echo $l_trackid ?></th>
                            <td><?php echo $order_track_id ?></td>
                        </tr>
                    </tbody>
                </table>
                <p class="text-dark mt-5">ท่านสามารถนำหมายเลขติดตามพัสดุไปตรวจสอบสถานะการจัดส่งได้ที่เว็บเพจของบริษํท <?php echo $order_track_name ?> ได้เลยค่ะ</p>
            </div>
        <?php } else { 
            echo $contact_detail;    
    } } ?>
